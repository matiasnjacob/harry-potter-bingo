...
7. Wait a few seconds for Xcode to refresh signing
8. If Xcode shows a `Fix Issue` button, click it
...
```text
com.matiasbinagora.oliharrypotterbingo.app
```
...
If the iPhone does not appear correctly in Xcode:

- unplug and reconnect the cable
- keep the iPhone unlocked
- reselect the device in Xcode after it reconnects
...
If you see multiple destinations, pick the physical iPhone rather than `Any iOS Device` or a simulator.
...
If build or launch fails, capture the exact Xcode error dialog because the next step depends on the message.

## 7. If the phone blocks app launch

If Xcode shows a message similar to:
...
1. open the profile for your Apple ID under `Developer App`
2. tap `Trust "<your Apple ID>"`
3. confirm trust
4. go back to Xcode and press `Cmd + R` again

If the developer profile is missing:

- keep the app installed from Xcode
- attempt one run from Xcode first
- then check `Settings > General > VPN & Device Management` again

After trust is granted, the app should be able to launch on the device.

## 8. Full end-to-end quick checklist

1. Add Apple ID in `Xcode > Settings > Accounts`
2. Open the FEATURE-009 project worktree
3. In `Signing & Capabilities`, choose your personal team
4. Keep automatic signing enabled
5. Confirm a unique bundle identifier
6. Connect the iPhone by cable and unlock it
7. Enable Developer Mode on the iPhone if needed
8. Select the physical iPhone as the run destination
9. Press `Cmd + R`
10. If prompted, trust the Developer App certificate on the iPhone
11. Press `Cmd + R` again
12. Confirm the app launches on the device

## 9. Known limitations of free provisioning
...
## 10. Validation target for this feature
